...
export async function triggerAlerts(monitor: { id: string, name: string }, incident: { id: string }, state: "UP" | "DOWN") {
    ...
    // ⚡ Bolt Performance Optimization: Process alerts concurrently instead of sequentially
    // Expected impact: Reduces total time to send N alerts from O(N * time_per_alert) to O(max(time_per_alert)).
