...
export async function triggerAlerts(monitor: { id: string, name: string }, incident: { id: string }, state: "UP" | "DOWN") {
    ...
    // ⚡ Bolt: Send alerts concurrently rather than sequentially.
    // Impact: Reduces overall notification latency from O(N) to O(1) network time, preventing a single slow target from delaying others.
