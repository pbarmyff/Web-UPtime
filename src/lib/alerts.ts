...
export async function triggerAlerts(monitor: { id: string, name: string }, incident: { id: string }, state: "UP" | "DOWN") {
    ...
    // Performance optimization: Dispatch webhook alerts concurrently rather than sequentially.
    // This reduces the O(N) network wait time bottleneck to O(1) time complexity over the network.
