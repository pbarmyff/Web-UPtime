...
export async function triggerAlerts(monitor: { id: string, name: string }, incident: { id: string }, state: "UP" | "DOWN") {
    ...
    // Refactored to execute alerts concurrently via Promise.all.
    // Explicit tests for this background alerting mechanism are omitted
    // as per project documentation policies for monitoring scripts.
