...
export async function triggerAlerts(monitor: { id: string, name: string }, incident: { id: string }, state: "UP" | "DOWN") {
    ...
    // ⚡ Bolt: Execute alert rules concurrently rather than sequentially
    // Reduces O(N) wait time where N is the number of rules. Each webhook failure is caught individually.
